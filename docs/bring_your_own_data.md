# Running the rolling forecast on your own data
...
You need an Anthropic API key. The key powers the commentary call (narrating the
forecast); all forecasting runs locally.
...
Paste your key into the sidebar, then open Forecast and click Write commentary.

## Adjusting drivers in the web app

The Forecast page lets you change any driver's method and value from the sidebar
without editing CSV files. Changes take effect when you click Re-forecast. The base
forecast is cached, so resetting to defaults is instant.
...
the Anthropic API under your own key: the computed P&L and driver detail the model
narrates. The forecasting itself runs locally. Nothing is sent to this project's
authors or the public app.
